	...
	{
		$("#pelunasan_start_date").html("1 Desember 2014");
		var today = new Date();
		...
		$("#pelunasan_end_date").html("31 Desember 2014");
	...
	// when I want to reset my data I call
		...
		$("#button").click(function() // function searching button
		{
			...
			$.ajax({
				type: "POST",
				url : "http://pln.ifabula.com/mobility_controller/display_pelunasan/"+start_date+"/"+end_date,
			...
			$.ajax({
			type: "POST",
			url : "http://pln.ifabula.com/mobility_controller/search_pelunasan_wilayah/"+start_date+"/"+end_date,
			data : "start_date="+start_date+"&end_date="+end_date,
			dataType: "json",
			success: function(result)
			{
				...
				for (var item in result)
				{
				  ...
				  if(result[item].PRODUK.toString() == 'POSTPAID')
				  {
					data_postpaid.push(total);
				  }
				  else if (result[item].PRODUK.toString() == 'PREPAID')
				  {
					data_prepaid.push(total);
				  }
				   else if (result[item].PRODUK.toString() == 'NONTAGLIS')
				  {
					data_nontaglis.push(total);
				  }
			...
			if (data_postpaid == undefined || data_postpaid.length == 0)
			 ...
			 if (data_nontaglis == undefined || data_nontaglis.length == 0)
			 {
				...
				$.ajax({
				type: "POST",
				url : "http://pln.ifabula.com/mobility_controller/search_cust_data/"+start_date+"/"+end_date,
				data : "start_date="+start_date+"&end_date="+end_date,
				dataType: "json",
				success: function(result)
				{
		...
		$.getJSON("http://pln.ifabula.com/mobility_controller/display_pelunasan_wilayah", function (result) 
		{	
		...
		$.getJSON("http://pln.ifabula.com/mobility_controller/display_lembar_pelunasan", function (result) 
		{	
		...
		$.getJSON("http://pln.ifabula.com/mobility_controller/display_tren_pelunasan", function (result) 
		{	
